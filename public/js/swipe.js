...
function swipeHandle(endSwipePos) {
	let absSwipePos = [
		Math.abs(startSwipePos[0] - endSwipePos[0]),
		Math.abs(startSwipePos[1] - endSwipePos[1]),
	];
	if (absSwipePos[0] < 50 || absSwipePos[0] < absSwipePos[1]) return;

	let pageArrow;
	if (
		startSwipePos[0] > endSwipePos[0] &&
		nowBible.chapter != Object.keys(bibles[nowBible.book]).length
	) {
		pageArrow = "right";
		nowBible.chapter++;
	} else if (
